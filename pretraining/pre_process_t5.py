...
def t5_masking(p):
	...
	id = 0
	while j < max_size:
		if rand.binomial(1, 0.1):
			d = min(rand.geometric(0.5), 5, 256-j)
			span = p[j:j+d]
			p = p[:j] + [get_token(id)] + p[j + d:]
			label += get_token(id) + " " + " ".join(span) + " "
			id += 1
		else:
			j += 1
	if id > 0:
...
if __name__ == "__main__":
	...
	for index in range(files):
		...
		for i, article in enumerate(tqdm(wiki_data, desc=f"file_{index}")):
			lines = article.split("\n")[2:]
	...
	print(len(dataset))
	pickle.dump(dataset, open("t5_data.pkl", 'wb'))
